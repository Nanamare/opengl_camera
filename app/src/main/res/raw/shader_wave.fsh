...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 out_image = texture2D(iChannel0, uv);
    ...
    float r = texture2D(iChannel0, uv + vec2(shift_val, 0.0) + vec2(0.005, 0)).r;
    float g = texture2D(iChannel0, uv + vec2(shift_val, 0.0) + vec2(-0.005, 0)).g;
    float b = texture2D(iChannel0, uv + vec2(shift_val, 0.0)).b;
