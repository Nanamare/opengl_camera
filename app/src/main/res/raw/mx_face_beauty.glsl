...
vec4 smoothes(vec2 coord, float lX, float lY, float radius,float stepX, float stepY) {
	...
	for (float i = startPos; i <= endPos; i += stepX) {
		vec4 tmpColor = texture2D(iChannel0, vec2(i, coord.y));
		color += tmpColor;
		num++;
	}
	...
	for (float i = startPos; i <= endPos; i += stepY) {
		vec4 tmpColor = texture2D(iChannel0, vec2(coord.x, i));
		color += tmpColor;
		num++;
	}
	color/=num;
...
void main() {
	...
	if (r > 0.3725 && g > 0.1569 && b > 0.0784 && r > b) {
		if (max(r, max(g,b)) - min(r, min(g, b)) > 0.05882) {
			if (abs(r - g) > 0.05882) {
				vec4 tmpColor = smoothes(texCoord, stepSizeX, stepSizeY, 10.0, stepSizeX * 5.0, stepSizeY * 5.0);
