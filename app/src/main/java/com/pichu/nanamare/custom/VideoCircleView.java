...
public class VideoCircleView extends View {

    private static final String TAG = VideoCircleView.class.getName();
    /**
     * 视频录制最长时间 秒
     */
     ...
     * 视频录制圆环默认颜色
     */
    private final int VIDEO_RECORD_DEFAULT_ANNULUS_COLOR = Color.parseColor("#1A000000");
    /**
     * 视频录制内圆默认颜色
     */
     ...
     * 视频录制进度默认颜色
     */
    private final int VIDEO_RECORD_DEFAULT_PROGRESS_COLOR = Color.parseColor("#6d89fc");
    ...
    public VideoCircleView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VideoCircleView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.VideoCircleView);
        mMaxTime = a.getInt(R.styleable.VideoCircleView_maxTime, VIDEO_RECORD_DEFAULT_MAX_TIME);
        mMinTime = a.getInt(R.styleable.VideoCircleView_minTime, VIDEO_RECORD_DEFAULT_MIN_TIME);

        excicleMagnification = a.getFloat(R.styleable.VideoCircleView_excicleMagnification
                , EXCICLE_MAGNIFICATION);
        innerCircleShrinks = a.getFloat(R.styleable.VideoCircleView_excicleMagnification
        ...
        mInitExCircleRadius = mExCircleRadius = a.getDimension(R.styleable.VideoCircleView_excircleRadius
                , VIDEO_RECORD_DEFAULT_EXCIRCLE_RADIUS);
        mInitInnerRadius = mInnerCircleRadius = a.getDimension(R.styleable.VideoCircleView_innerCircleRadius
                , VIDEO_RECORD_DEFAULT_INNER_CIRCLE_RADIUS);

        mAnnulusColor = a.getColor(R.styleable.VideoCircleView_annulusColor
                , VIDEO_RECORD_DEFAULT_ANNULUS_COLOR);
        mInnerCircleColor = a.getColor(R.styleable.VideoCircleView_innerCircleColor
                , VIDEO_RECORD_DEFAULT_INNER_CIRCLE_COLOR);
        mProgressColor = a.getColor(R.styleable.VideoCircleView_progressColor
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mInnerCircleRadius > mExCircleRadius) {
            ...
            Logger.INSTANCE.d(TAG, "mInnerCircleRadius == mExCircleRadius 你将看不到进度条");
        }
    }
    ...
    static class MHandler extends android.os.Handler {
        ...
        private WeakReference<VideoCircleView> weakReference = null;

        public MHandler(VideoCircleView controlView) {
            weakReference = new WeakReference<VideoCircleView>(controlView);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            final VideoCircleView videoCircleView = weakReference.get();
            switch (msg.what) {
                case VideoCircleView.MSG_START_LONG_RECORD:
                    if (videoCircleView.onRecordListener != null) {
                        videoCircleView.onRecordListener.OnRecordStartClick();
                    }
                    //内外圆动画，内圆缩小，外圆放大
                    videoCircleView.startAnimation(videoCircleView.mExCircleRadius,
                            videoCircleView.mExCircleRadius * videoCircleView.excicleMagnification,
                            videoCircleView.mInnerCircleRadius,
                            videoCircleView.mInnerCircleRadius * videoCircleView.excicleMagnification);
                    break;
            }
        }
    }
